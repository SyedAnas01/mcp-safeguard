...
OWASP classified **MCP Tool Poisoning** as an official 2026 attack category. This table shows how mcp-safeguard's 132 detection rules — prompt injection (15), credentials (31), tool poisoning (11), SSRF (3), OAuth scope risk (7), and source-audit (20) — plus 29 endpoint path probes and 12 dangerous-port checks cover OWASP's identified threat categories. This count is generated from the code itself (the `security://rules` MCP resource) rather than hand-maintained here.
...
### Category VI — Source-Level Audit (SRC-001 to SRC-020)
...
per-rule table. Not run by default (`scan`); invoked explicitly via
