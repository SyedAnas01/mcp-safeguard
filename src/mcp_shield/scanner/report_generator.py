...
def generate_scan_summary(
    ...
    all_findings: list[Any] = (
        injection_findings + credential_findings + endpoint_findings + tool_poisoning_findings
    )
...
def generate_html_report(report: SecurityReport) -> str:
        ...
        for f in findings:
            ...
            for col in columns:
                ...
                if col == "severity":
                    ...
                    row_cells.append(f"<td>{escaped[:200]}</td>")
    ...
    for tp in report.tool_risk_profiles:
        ...
        tool_rows.append(
            f"<tr>"
            f"<td><strong>{tp.tool_name}</strong></td>"
            ...
            f"<td>{'<br>'.join(tp.risk_factors[:3])}</td>"
            f"</tr>"
        )
    tool_table_rows = (
...
<title>mcp-shield Security Report — {s.target}</title>
<style>
...
<body>
<div class="header">
  <h1>🛡️ mcp-shield Security Report</h1>
  <p>Target: {s.target} &nbsp;|&nbsp; Scan ID: {s.scan_id} &nbsp;|&nbsp; {s.scan_time}</p>
...
<div class="footer">
  Generated by <strong>mcp-safeguard v0.3.0</strong> &mdash; <a href="https://github.com/SyedAnas01/mcp-safeguard">github.com/SyedAnas01/mcp-safeguard</a>
...
def generate_text_report(report: SecurityReport) -> str:
    ...
    lines.append("")
    lines.append("=" * 70)
    lines.append("  Generated by mcp-safeguard v0.3.0")
    lines.append("=" * 70)
    return "\n".join(lines)
